...
def HTSPS(barriers, N, prepro = True, log = False, timeLimit = 7200, init = False):
    # N = [N1, N2]
    ...
    # L, U = estima_det(N[0], barriers[1])
    
    # MODEL.addConstrs((1 - alpha00[a, e])*(estima_det(N[abs(a)-1], barriers[e])[0]) <= determinant([P[a, 0, 0], P[a, 0, 1]], barriers[e][0], barriers[e][1]) for a, e in alpha00.keys())
    # MODEL.addConstrs(determinant([P[a, 0, 0], P[a, 0, 1]], barriers[e][0], barriers[e][1]) <= estima_det(N[abs(a)-1], barriers[e])[1]*alpha00[a, e] for a, e in alpha00.keys())
    ...
    # Restriccion 1
    MODEL.addConstrs(gp.quicksum(y[v, i, vn, j] for v, i in V if (v, i, vn, j) in EN) >= 1 for vn, j in VN)
    ...
    for vn, i in VN:
        if vn <= -2:
            MODEL.addConstr(gp.quicksum(g[vn, i, v, j] for v, j in V if (vn, i, v, j) in EN) - gp.quicksum(g[v, j, vn, i] for v, j in V if (v, j, vn, i) in EN) == 1)
