...
def HTSPS_with_prepro4(barriers, N, log = False, timeLimit = 7200, init = False):
    # N = [N1, N2]
    ...
    PN = MODEL.addVars(P_index, vtype = GRB.CONTINUOUS, name = 'PN')
    P1 = MODEL.addVars(alpha_index, vtype = GRB.CONTINUOUS, name = 'P1')
    param1 = MODEL.addVars(delta_index, vtype = GRB.CONTINUOUS, lb = -GRB.INFINITY, ub = GRB.INFINITY, name = 'param1')
    P2 = MODEL.addVars(alpha_index, vtype = GRB.CONTINUOUS, lb = -GRB.INFINITY, ub = GRB.INFINITY,  name = 'P2')
    param2 = MODEL.addVars(delta_index, vtype = GRB.CONTINUOUS, name = 'param2')
    # dP1P2 = MODEL.addVars(delta_index, vtype = GRB.CONTINUOUS, lb = 0.0, name = 'dP1P2')
    # difP1P2 = MODEL.addVars(alpha_index, vtype = GRB.CONTINUOUS, lb = 0.0, name = 'difP1P2')    
    ...
    # alpha-C
    for a, dim in PN.keys():
        PN[a, dim] = N[(abs(a)-1)].center[dim]
    
    # param2[-1, 4, 1, 3] = -0.75
    # param2[-1, 3, 1, 2] = -5
    # MODEL.addConstr(PN[-1, 1] == 0)
    ...
    for a, b, c, d in beta.keys():
        ...
        MODEL.addConstr(param2[a, b, c, d]*(x4-x3)*(y2-y1)-(y4-y3)*(x2-x1) <= (y3-y1)*(x2-x1)-(x3-x1)*(y2-y1))
        # MODEL.addConstr(x1 + param1[a, b, c, d]*(x2 - x1) == x3 + param2[a, b, c, d]*(x4 - x3))
        # MODEL.addConstr(y1 + param1[a, b, c, d]*(y2 - y1) == y3 + param2[a, b, c, d]*(y4 - y3))
        
        # MODEL.addConstr(param2[a, b, c, d] <= U*beta[a, b, c, d])
        # MODEL.addConstr(param2[a, b, c, d] >= L*gamma[a, b, c, d])
        
        
        #
        # MODEL.addConstrs(P2[a, b, c, d, dim] == barriers[d][0][dim]*param2[a, b, c, d] + barriers[d][1][dim]*(1 - param2[a, b, c, d]) for dim in range(2))
        #
        # MODEL.addConstrs(difP1P2[a, b, c, d, dim] >= P1[a, b, c, d, dim] - P2[a, b, c, d, dim] for dim in range(2))
        # MODEL.addConstrs(difP1P2[a, b, c, d, dim] >= P1[a, b, c, d, dim] - P2[a, b, c, d, dim] for dim in range(2))
        #
        #
        # MODEL.addConstr(difP1P2[a, b, c, d, 0]*difP1P2[a, b, c, d, 0] + difP1P2[a, b, c, d, 1]*difP1P2[a, b, c, d, 1] <= dP1P2[a, b, c, d]*dP1P2[a, b, c, d])
        
        # MODEL.addConstr(dP1P2[a, b, c, d] <= -1e-5 + U*(1 - delta[a, b, c, d]))
        # MODEL.addConstr(dP1P2[a, b, c, d] >= -1e-5 + L*(delta[a, b, c, d]))
        ...
        #
        #
        # MODEL.addConstr((y3-y1)*(x2-x1)-(x3-x1)*(y2-y1) - (x4-x3)*(y2-y1)-(y4-y3)*(x2-x1) >= L*gamma[a, b, c, d])
        # MODEL.addConstr((y3-y1)*(x2-x1)-(x3-x1)*(y2-y1) - (x4-x3)*(y2-y1)-(y4-y3)*(x2-x1) >= L*(gamma[a, b, c, d]))
        #
        MODEL.addConstr(delta[a, b, c, d] >= beta[a, b, c, d] + gamma[a, b, c, d] - 1)
    ...
    for a, b, c in epsilon.keys():
        ...
        # MODEL.addConstr(1- epsilon[a, b, c] <= len(barriers)*delta.sum(a, b, c, '*'))
    ...
    for a, dim in PN.keys():
        ...
        if type(entorno) is Circulo:
            MODEL.addConstr(dif_inside[a, dim] >= PN[a, dim] - N[abs(a)-1].center[dim])
            MODEL.addConstr(dif_inside[a, dim] >= N[abs(a)-1].center[dim] - PN[a, dim])
        ...
        if type(entorno) is Poligonal:
            MODEL.addConstrs(PN[a, dim] == landa[a]*entorno.V[0][dim] + (1-landa[a])*entorno.V[1][dim] for dim in range(2))
        
    
    # MODEL.addConstr(y[-1, 2, 1] >= 0.5)
    # MODEL.addConstr(y[-2, 2, 1] >= 0.5)
    # MODEL.addConstr(PN[-1, 0] == 30)
    # MODEL.addConstr(PN[-1, 1] == 10)
    ...
    for index in dif_index:
        if len(index) == 4:
            ...
            # print((a, b, c))
            if a < 0:
                MODEL.addConstr(dif[index] >= PN[a, dim] - barriers[b][c][dim])
                MODEL.addConstr(dif[index] >= barriers[b][c][dim] - PN[a, dim])
            
            elif c < 0:
                MODEL.addConstr(dif[index] >= PN[c, dim] - barriers[a][b][dim])
                MODEL.addConstr(dif[index] >= barriers[a][b][dim] - PN[c, dim])
            #
            # if a >= 0:
            #     MODEL.addConstrs(dif[index, dim] >= PN[c, dim] - barriers[a][b][dim] for dim in range(2))
            #     MODEL.addConstrs(dif[index, dim] >= barriers[a][b][dim] - PN[c, dim] for dim in range(2))
    ...
    objective = gp.quicksum(p[index] for index in p.keys()) + gp.quicksum(dist[index]*y[index] for index in EB) #+ gp.quicksum(U*dP1P2[index] for index in dP1P2.keys())
