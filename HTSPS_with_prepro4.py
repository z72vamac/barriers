...
def HTSPS_with_prepro4(barriers, N, log = False, timeLimit = 7200, init = False):
    ...
    param2 = MODEL.addVars(, vtype = GRB.CONTINUOUS, name = 'param2')
    ...
    for a, dim in PN.keys():
        ...
        if type(neighborhood) is Poligonal:
            MODEL.addConstrs(PN[a, dim] == landa[a]*neighborhood.V[0][dim] + (1-landa[a])*neighborhood.V[1][dim] for dim in range(2))
