...
\end{prop}
\begin{proof}
Note that the point selected in $N_S$ in an optimal solution for \SPPN \ must the one that gives the minimum distance to the point of the first visited barrier in the optimal solution. Therefore, $N^*$ must be composed, at most, by points in the set $\{P_S(P_B^i)=\argmin_{P\in N_S} \|P_B^i - P\|:P_B^i\in V_{\mathcal B}\}$.
\end{proof}
