...
def HTSPS_with_prepro2(barriers, N, log = False, timeLimit = 7200, init = False):
    # N = [N1, N2]
    ...
    for a, b, c, d, e, f in alpha.keys():
        ...
        elif (e == 0 and f == 1) or (e == 1 and f == 1):
    ...
    for a, b, c in epsilon.keys():
        ...
        MODEL.addConstr(len(barriers) - delta.sum(a, b, c, '*') >= len(barriers)*epsilon[a, b, c])
