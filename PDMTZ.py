...
def PDMTZ(datos):
    ...
    if datos.init:
        sols = MODEL.load('./soluciones/PDMTZ-Heur.sol')
        # try:
        #     z_init, xL_init, xR_init, ugi_init, vgi_init, results = PDMTZ_heuristic(datos)
        #
        ...
        #     for g in T_index_prima:
        #         for dim in range(2):
        #             xL[g, dim].start = xL_init[g][dim]
        #             xR[g, dim].start = xR_init[g][dim]
        #
        #     for g, i in ugi.keys():
        #         ugi[g, i].start = ugi_init[g, i]
        #
        # except:
